...
int main() {
    BenchMarkParser bp("QASM_example/example.qasm");
    ...
    graph = Graph(bp.getLogiNumber(),circuit);    
    ...
    try {
        ...
        int numQubits = bp.getLogiNumber();
        ...
        for (auto &[edge, var] : C) {
            ...
            for (int p = 0; p < numPartitions; p++) {
                ...
                model.addConstr(var >= m[u][p] - m[v][p], "Cuv_upper_" + to_string(u) + "_" + to_string(v)+ "_" + to_string(p));
                model.addConstr(var >= m[v][p] - m[u][p], "Cuv_lower_" + to_string(u) + "_" + to_string(v)+ "_" + to_string(p));
                model.addConstr(var <= m[u][p] + m[v][p], "Cuv_sum_" + to_string(u) + "_" + to_string(v)+ "_" + to_string(p));
                model.addConstr(var <= 2 - m[u][p] - m[v][p], "Cuv_2_minus_sum_" + to_string(u) + "_" + to_string(v)+ "_" + to_string(p));
            }
        ...
        GRBLinExpr W_C = 0; // Σ(z[u,v,p] = 1)
        GRBLinExpr G_C = 0; // Σ(C[u,v] = 1) - Σ(z[u,v,p] = 1)
        GRBLinExpr obj = 0;
         for (auto &[key, var] : z) {
            W_C += var; // z[u,v,p]
        }
        ...
        G_C -= W_C;
        // obj = pow((double)4,W_C)+pow((double)6,G_C);
        obj = 4*W_C + 6*G_C;
        ...
        if (model.get(GRB_IntAttr_Status) == GRB_OPTIMAL) {
            ...
            model.computeIIS();
            model.write("infeasible.ilp"); // 保存 IIS 到文件中
        ...
        for (int v = 0; v < numQubits * layers; v++) {
            for (int p = 0; p < numPartitions; p++) {
                cout << "m[" << v << "][" << p << "] = " 
                    << m[v][p].get(GRB_DoubleAttr_X) << endl;
            }
        }
