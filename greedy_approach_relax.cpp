...
// todo list : 只要交換後不超過就好 若是要保留wire cutting可能會多一顆qubit應該不能把這個relax掉 
// 1. initial partition , METIS KAHYPAR KAHIP
// 2. 在換的時候每個 partition 至少要有 1 顆 qubit 的空間才可進行wire cutting?
// 2.1 wire cutting 獨立成一個 time slot ? 過後可以重複使用 qubit ?
...
void greedyApproach(QuantumCircuit circuit,int qubitNumber,int maxQubitCount){
    int gateCutting = 0;
    int wireCutting = 0;
    // sorted with dag
    ...
    QuantumGate qg;
    vector<vector<int>> potentialPartitionArr;
    PartitionManager pm = PartitionManager(circuit,qubitNumber,maxQubitCount);
...
int main(int argc, char* argv[]){
    ...
    greedyApproach(circuit,qubitNumber,maxQubitCount);
